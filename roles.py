    ...
    async def _act(self) -> Message:
        ...
        if isinstance(todo, WriteTestCode):
            ...
            for memory in self.get_memories():
                if memory.cause_by == "actions.WriteTestCase":
                    test = memory.content
            code = read_file(IMPLEMENT_FILE_PATH)
            resp = await todo.run(code=code, test=test, fpath=IMPLEMENT_TEST_FILE_PATH)
            msg = Message(content=resp, role=self.profile, cause_by=type(todo))
        ...
        # 运行测试代码
            ...
            compile_error = self.get_memories(k=1)[0]
            resp += compile_error.content
